...
namespace Decrypter
{ 
        public class Decrytper
        {
        private static string m_ClientFileLocation = Application.StartupPath;    //AppDomain.CurrentDomain.BaseDirectory; // App's working directory
            private static string CLIENT = "client.exe";  // Name of un-decrypted client file
            ...
            public static void StartDecrytpion()
            {
            ...
            private static void ReadClientFile()
            {
                try
                {
            ...
            private static void MultiClientPatch()
            {
            ...
            private static void RemoveEncryption(byte[] InClient, long InClientLength)
            {
                ...
                {
                    MessageBox.Show( "Can't find a login signature in this file ???");
                }
                else
                {
                    if (CryptPos != -1)
                    {
                        ...
                        {
                            bytes[JNZPos + 1] = 0x84;
                             Debug.Print(string.Format("Patching with JE (0x0F 0x84) - (15 132) @{0:X} - ({1})", JNZPos, JNZPos.ToString()));
                        }
                        else if (JNZPos > JEPos)
                        {
                            bytes[JNZPos + 1] = 0x85;
                            Debug.Print(string.Format("Patching with JNZ (0x0F 0x85) - (15 133) @{0:X} - ({1})", JEPos, JEPos.ToString()));
                        }
                    }
                    else if (CryptPosNew != -1)
                    {
                        bytes[CryptPosNew + 4] = 0xEB;
                        Debug.Print(string.Format("Patching with (0xEB) - (235) @{0:X} - ({1})", (CryptPosNew + 4), (CryptPosNew + 4).ToString()));
                        NewClient = 1;
                    }
                }
                ...
                */
                ...
                {
                    MessageBox.Show("Can't find the blowfish signature");
                }
                else
                {
                    bytes[CmpPos + 1] = 0xC0;
                    Debug.Print(string.Format("Patching with CMP (0xC0) - (192) @{0:X} - ({1})", CmpPos, CmpPos.ToString()));
                }
                ...
                */
                ...
                {
                    MessageBox.Show("Can't find old OR new Twofish signatures");
                }
                else
                {
                    ...
                    {
                        bytes[GJEPos + 1] = 0x85;
                        Debug.Print(string.Format("Patching (old TF) with JNZ (0x85) - (133) @{0:X} - ({1})", (GJEPos + 1), (GJEPos + 1).ToString()));
                    }
                    else if (TFGamecryptPosNew != -1)
                    {
                        bytes[TFGamecryptPosNew] = 0xEB;
                        Debug.Print(string.Format("Patching (new TF) with (0xEB) - (235) @{0:X} - ({1})", TFGamecryptPosNew, TFGamecryptPosNew.ToString()));
                    }
                }
                ...
                */
                ...
                {
                    MessageBox.Show("Can't find any MD5 Decrypt signatures ???");
                }
                else
                {
                    if (NewClient == -1)
                    {
                        ...
                        Debug.Print(string.Format("Patching old MD5 with CMP (0x3B) - (59) @{0:X} - ({1})", DectestPos, DectestPos.ToString()));
                    }
                    else if (NewClient == 1)
                    {
                        bytes[DecryptPosNew + 2] = 0xEB;
                        Debug.Print(string.Format("Patching (new MD5 (D2+2)) with (0xEB) - (235) @{0:X} - ({1})", DecryptPosNew, DecryptPosNew.ToString()));
                    }
                }
            }

            private static int ScanClient(int FlexByte, byte[] signature, byte[] client, long client_length, int sig_length, int startat)
            {
                int Count = 0, i = 0, j = 0;
            ...
            private static bool GetMyBool(int a, int b)
            {
                int TheBool = a & b;
            ...
            private static byte GetMyByte(int flex)
            {
